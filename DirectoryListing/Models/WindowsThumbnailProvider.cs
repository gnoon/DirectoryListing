...
namespace DirectoryListing.Models
{
    ...
    public class WindowsThumbnailProvider
    {
        ...
        public static string GetThumbnailBase64(string fileName)//, int width, int height, ThumbnailOptions options)
        {
            int width = Convert.ToInt32(ConfigurationManager.AppSettings["iconwidth"]);
            int height = Convert.ToInt32(ConfigurationManager.AppSettings["iconheight"]);
            var options = ThumbnailOptions.BiggerSizeOk;
            using (var bmp = GetThumbnail(fileName, width, height, options))
            {
                using (var mem = new MemoryStream())
                {
                    bmp.Save(mem, ImageFormat.Png);
                    var base64 = Convert.ToBase64String(mem.GetBuffer());
                    return base64;
                }
            }
        }
